...
struct allow_leading_plus_sign {
  struct json_value_s *value;
};

UTEST_F_SETUP(allow_leading_plus_sign) {
  const char payload[] = "{\"foo\" : +1}";
  utest_fixture->value =
      json_parse_ex(payload, strlen(payload),
                    json_parse_flags_allow_leading_plus_sign, 0, 0, 0);
...
UTEST_F_TEARDOWN(allow_leading_plus_sign) {
  ...
  ASSERT_STREQ("1", number->number);
  ASSERT_EQ(strlen("1"), number->number_size);

  free(value);
}

UTEST_F(allow_leading_plus_sign, read_write_pretty_read) {
  size_t size = 0;
...
UTEST_F(allow_leading_plus_sign, read_write_minified_read) {
  size_t size = 0;
